...
class Details extends Component {
    ...
    async componentDidMount(){
        let restlId = this.props.location.search.split('=')[1];
        let response = await axios.get(`${url}/${restlId}`)
        let menuResponse = await axios.get(`${menuUrl}/${restlId}`)
